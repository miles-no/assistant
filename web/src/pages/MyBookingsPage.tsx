...
import { Card, LoadingSpinner, Badge, Button, ConfirmModal } from '@/components/ui';
import { useBookings, useCancelBooking } from '@/hooks/useBookings';
import { Calendar, Clock, MapPin, DoorOpen, Trash2 } from 'lucide-react';
import type { Booking, Room, Location, User } from '@/api-generated/types.gen';
...
export default function MyBookingsPage() {
  ...
  const upcomingBookings = bookings?.filter(
    (b) => b.startTime && new Date(b.startTime) > now && b.status !== 'CANCELLED'
  );
  const pastBookings = bookings?.filter(
    (b) => (b.endTime && new Date(b.endTime) <= now) || b.status === 'CANCELLED'
  );
                ...
                <>
                  ...
                  {booking.room.location && (
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        {booking.room.location.name} - {booking.room.location.city}
                      </span>
                    </div>
                  )}
                </>
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <p className="mt-2 text-gray-600">
            Manage your meeting room reservations ({bookings?.length || 0} total)
          </p>
        </div>
        ...
        <div>
          ...
          {!upcomingBookings || upcomingBookings.length === 0 ? (
            <Card className="text-center">
              ...
              <h3 className="mt-4 text-lg font-semibold text-gray-900">No upcoming bookings</h3>
              <p className="mt-2 text-gray-600">You don&apos;t have any upcoming reservations.</p>
