...
test.describe('Navigation & Layout', () => {
  ...
  test('should display navigation items', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByRole('link', { name: /locations/i })).toBeVisible();
    await expect(authenticatedPage.getByRole('link', { name: /my bookings/i })).toBeVisible();
    await expect(authenticatedPage.getByRole('link', { name: /settings/i })).toBeVisible();
  });

  test('should not show management nav for regular user', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/dashboard');

    await expect(authenticatedPage.getByRole('link', { name: /management/i })).not.toBeVisible();
  });
  ...
  test('should show management nav for admin', async ({ adminPage }) => {
    ...
    await expect(sidebar.getByRole('link', { name: /management/i })).toBeVisible();
  });
  ...
  test('should highlight active route', async ({ authenticatedPage }) => {
    ...
    const locationsLink = authenticatedPage.getByRole('link', { name: /locations/i });
  ...
  test('should have mobile menu button on small screens', async ({ authenticatedPage }) => {
    await authenticatedPage.setViewportSize({ width: 375, height: 667 }); // Mobile size
    await authenticatedPage.goto('/dashboard');

    // Mobile menu button should be visible
    const menuButton = authenticatedPage.getByRole('button').filter({ has: authenticatedPage.locator('svg') }).first();
