...
test.describe("Locations", () => {
	test("should display locations page", async ({ authenticatedPage }) => {
		...
		await expect(
			authenticatedPage.getByRole("heading", { name: /office locations/i }),
		).toBeVisible();
		await expect(
			authenticatedPage.getByText(/browse our \d+ office locations/i),
		).toBeVisible();
	});

	test("should display location cards with data", async ({
		authenticatedPage,
	}) => {
		await authenticatedPage.goto("/locations");
	...
	test("should have search functionality", async ({ authenticatedPage }) => {
		...
		await expect(
			authenticatedPage.getByPlaceholder(/search locations/i),
		).toBeVisible();
	});
	...
	test("should filter locations by search", async ({ authenticatedPage }) => {
		...
		await authenticatedPage
			.getByPlaceholder(/search locations/i)
			.fill("Stavanger");
	...
	test("should display results count", async ({ authenticatedPage }) => {
		...
		await expect(
			authenticatedPage.getByText(/showing \d+ of \d+ locations/i),
		).toBeVisible();
	});

	test("should navigate to location details on click", async ({
		authenticatedPage,
	}) => {
		await authenticatedPage.goto("/locations");
	...
	test("should display empty state when no results", async ({
		...
		await authenticatedPage
			.getByPlaceholder(/search locations/i)
			.fill("NonExistentLocation");

		await expect(
			authenticatedPage.getByText("No locations found"),
		).toBeVisible();
		await expect(
			authenticatedPage.getByText(/try adjusting your search/i),
		).toBeVisible();
	});
});
...
test.describe("Location Detail", () => {
	test("should display location detail page", async ({ authenticatedPage }) => {
		...
		await expect(
			authenticatedPage.getByRole("heading", { name: "Stavanger Office" }),
		).toBeVisible();
		await expect(authenticatedPage.getByText("Tech Street 123")).toBeVisible();
		await expect(
			authenticatedPage.getByText(/timezone: europe\/oslo/i),
		).toBeVisible();
	});
	...
	test("should display location description", async ({ authenticatedPage }) => {
		...
		await expect(
			authenticatedPage.getByText("Modern office in downtown Stavanger"),
		).toBeVisible();
	});

	test("should display meeting rooms section", async ({
		authenticatedPage,
	}) => {
		await authenticatedPage.goto("/locations/1");

		await expect(
			authenticatedPage.getByRole("heading", { name: /meeting rooms/i }),
		).toBeVisible();
		await expect(
			authenticatedPage.getByText("Conference Room A"),
		).toBeVisible();
		await expect(
			authenticatedPage.getByText(/capacity: 10 people/i),
		).toBeVisible();
	});
	...
	test("should handle non-existent location gracefully", async ({
		...
		await expect(
			authenticatedPage.getByText(/failed to load location details/i),
		).toBeVisible();
		await expect(
			authenticatedPage.getByRole("button", { name: /back to locations/i }),
		).toBeVisible();
	});
});
