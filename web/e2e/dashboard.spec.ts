...
test.describe('Dashboard', () => {
  ...
  test('should display dashboard for authenticated user', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByRole('heading', { name: /welcome back, test!/i })).toBeVisible();
    await expect(authenticatedPage.getByText(/manage your office bookings/i)).toBeVisible();
  });
  ...
  test('should display quick action cards', async ({ authenticatedPage }) => {
    ...
    await expect(main.getByRole('heading', { name: 'Management' })).not.toBeVisible();
  });
  ...
  test('should display management card for admin', async ({ adminPage }) => {
    ...
    await expect(main.getByRole('heading', { name: 'Management' })).toBeVisible();
  });

  test('should display stats cards', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/dashboard');

    await expect(authenticatedPage.getByText('Upcoming Bookings')).toBeVisible();
  ...
  test('should navigate to locations from quick action', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/dashboard');
  ...
  test('should navigate to bookings from quick action', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/dashboard');
  ...
  test('should display welcome card with CTAs', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/dashboard');

    await expect(authenticatedPage.getByText(/welcome to miles booking!/i)).toBeVisible();
    await expect(authenticatedPage.getByRole('button', { name: /explore locations/i })).toBeVisible();
    await expect(authenticatedPage.getByRole('button', { name: /view bookings/i })).toBeVisible();
  });
});
