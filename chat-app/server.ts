...
import { getProvider, getProviderInfo } from "./llm-providers";
	...
	for (const pattern of offTopicPatterns) {
		if (pattern.test(lowerMessage)) {
			return {
				valid: false,
				response:
					"I'm a booking system assistant. I can only help with room bookings, availability, and reservations. How can I help with that?",
			};
		}
	}
...
async function getMCPResources(): Promise<MCPResource[]> {
	try {
		const response = await axios.get<{ resources?: MCPResource[] }>(
			`${MCP_API_URL}/resources`,
		);
		return response.data.resources || [];
	} catch (error) {
		console.error(
			"Error fetching MCP resources:",
			error instanceof Error ? error.message : String(error),
		);
		return [];
	}
...
// Read MCP resource
async function readMCPResource(resourcePath: string): Promise<{
	contents: Array<{ uri: string; mimeType: string; text: string }>;
}> {
	try {
		const response = await axios.get(
			`${MCP_API_URL}/resources/${resourcePath}`,
		);
		return response.data;
	} catch (error) {
		console.error(
			`Error reading resource ${resourcePath}:`,
			error instanceof Error ? error.message : String(error),
		);
		return {
			contents: [
...
function formatToolsForPrompt(tools: MCPTool[]): string {
	return tools
		.map((tool) => {
...
function formatResourcesForPrompt(resources: MCPResource[]): string {
	return resources
		.map((resource) => {
			// Convert miles://path to a tool-like name (e.g., "read_locations")
			const resourcePath = resource.uri
				.replace("miles://", "")
				.replace(/[{}]/g, "");
...
function parseToolCalls(text: string): ParsedToolCalls {
	const toolCalls: ToolCall[] = [];
	const resourceReads: ResourceRead[] = [];
		...
		if (toolName.startsWith("read_")) {
			...
			try {
				...
				const params =
					paramsStr === "" || paramsStr === "{}"
						? {}
						: (JSON.parse(paramsStr) as Record<string, unknown>);
				toolCalls.push({ toolName, params, isResource: false });
			} catch (error) {
				console.error(
					"Error parsing tool call params:",
					error,
					"paramsStr:",
					paramsStr,
				);
...
User's message: ${message}`;
		...
		if (toolCalls.length > 0 || resourceReads.length > 0) {
			...
			// Combine all results
			const allResults: (ToolResult | ResourceResult)[] = [
				...toolResults,
				...resourceResults,
			];
			const resultsText = allResults
				.map((item) => {
					if (item.type === "tool") {
						const text =
							item.result.content?.[0]?.text || JSON.stringify(item.result);
						return `Tool ${item.toolName} result: ${text}`;
					}
					const text =
						item.result.contents?.[0]?.text || JSON.stringify(item.result);
			...
			const hasMultipleItems =
				resultsText.match(/\[.*,.*\]/s) ||
				resultsText.includes('"rooms"') ||
				resultsText.includes('"bookings"');
...
app.get("/api/mcp/info", async (_req: Request, res: Response) => {
	try {
