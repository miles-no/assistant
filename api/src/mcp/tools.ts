			...
			inputSchema: {
				...
				properties: {
					...
					message: {
						type: "string",
						description:
							"Free-form feedback message describing the issue or suggestion",
					},
				},
...
async function suggestBookingTime(args: any) {
	...
	let suggestedStart = new Date(
		Math.max(startSearchDate.getTime(), Date.now()),
	);
	...
	for (let attempt = 0; attempt < 100; attempt++) {
		...
		const hasConflict = bookings.some((booking) => {
			return (
				(booking.startTime <= suggestedStart &&
					booking.endTime > suggestedStart) ||
