...
async function main() {
	...
	const _haugesund = await prisma.location.upsert({
		where: { id: "haugesund" },
		update: {},
		create: {
			id: "haugesund",
	...
	const _bergen = await prisma.location.upsert({
		where: { id: "bergen" },
		update: {},
		create: {
			id: "bergen",
	...
	const _alesund = await prisma.location.upsert({
		where: { id: "alesund" },
		update: {},
		create: {
			id: "alesund",
	...
	const _innlandet = await prisma.location.upsert({
		where: { id: "innlandet" },
		update: {},
		create: {
			id: "innlandet",
	...
	const _lithuania = await prisma.location.upsert({
		where: { id: "lithuania" },
		update: {},
		create: {
			id: "lithuania",
	...
	await prisma.roomFeedback.create({
		data: {
			...
			userId: user1.id,
			message:
				"The projector remote is missing. We had to use the buttons on the projector itself.",
			status: "OPEN",
		},
	});
	...
	await prisma.roomFeedback.create({
		data: {
			...
			userId: user1.id,
			message:
				"Would be great to have a coffee machine in this room for longer meetings!",
			status: "OPEN",
		},
	});
	...
	console.log(
		"  Manager (Stavanger): manager.stavanger@miles.com / password123",
	);
