...
async function _getLastOutput(page) {
...
test.describe("IRIS Terminal - Happy Paths", () => {
	...
	test("03 - Display bookings command", async ({ page }) => {
		...
		const noBookings = outputText.includes(
			"[WARNING] No active bookings found",
		);
	...
	test("07 - Check room availability", async ({ page }) => {
		...
		const hasAvailability =
			outputText.includes("[OK] Availability data retrieved") ||
	...
	test("14 - IRIS personality responses", async ({ page }) => {
		...
		const hasColdResponse =
			outputText.includes("irrelevant") ||
