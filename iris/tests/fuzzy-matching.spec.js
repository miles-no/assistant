...
test.describe('IRIS Fuzzy Matching - Room Name Variations', () => {
  ...
  test('01 - Fuzzy match: "conference" should work like "Conference Room A"', async ({ page }) => {
    await login(page);
    ...
    const validResponse = output.includes('Conference Room A') ||
  ...
  test('02 - Fuzzy match: "focus" should work like "Focus Pod B"', async ({ page }) => {
    await login(page);
    ...
    const validResponse = output.includes('Focus Pod B') ||
  ...
  test('03 - Fuzzy match: "meeting" should work like "Meeting Room C"', async ({ page }) => {
    await login(page);
    ...
    const validResponse = output.includes('Meeting Room C') ||
                         output.includes('available') ||
                         output.includes('Room');
  ...
  test('04 - Fuzzy match: "virtual" should work like "Virtual Lab D"', async ({ page }) => {
    await login(page);
    ...
    const validResponse = output.includes('Virtual Lab D') ||
  ...
  test('05 - Fuzzy match: "presentation" should work like "Presentation Suite"', async ({ page }) => {
    await login(page);
    ...
    const validResponse = output.includes('Presentation Suite') ||
                         output.includes('available') ||
                         output.includes('Room');
  ...
  test('06 - Fuzzy match: misspelling "focuss" should still match "Focus Pod B"', async ({ page }) => {
    await login(page);
  ...
  test('07 - Exact match should still work: "Conference Room A"', async ({ page }) => {
    await login(page);
  ...
  test('08 - Case insensitive: "CONFERENCE ROOM A" should work', async ({ page }) => {
    await login(page);
  ...
  test('09 - Multiple misspellings: "virtul lab" should work', async ({ page }) => {
    await login(page);
  ...
  test('10 - Fuzzy match in natural language query', async ({ page }) => {
    ...
    await sendCommand(page, 'can I book focus for a meeting tomorrow afternoon?');
