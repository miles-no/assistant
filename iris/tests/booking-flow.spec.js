...
test.describe("IRIS Booking Flow - Happy Paths", () => {
	...
	test("03 - Regular user can create a booking for tomorrow", async ({
		page,
	}) => {
		await login(page, REGULAR_USER);
		...
		await sendCommand(
			page,
			`book Teamrommet tomorrow at ${timeStr} for 1 hour`,
		);
	...
	test("05 - Regular user receives helpful error for unavailable room", async ({
		page,
	}) => {
		await login(page, REGULAR_USER);
	...
	test("07 - Natural language booking query works", async ({ page }) => {
		...
		await sendCommand(
			page,
			"can I book conference room for tomorrow afternoon?",
		);
	...
	test("09 - Bulk cancel with no bookings shows helpful message", async ({
		page,
	}) => {
		await login(page, REGULAR_USER);
		...
		const validResponse =
			output.includes("[OK] Cancellation") ||
	...
	test("16 - Session persists after commands", async ({ page }) => {
		...
		const output = await getTerminalOutput(page);
		const validResponse =
			output.includes("booking") || output.includes("No active bookings");
		expect(validResponse).toBeTruthy();
	});
});
