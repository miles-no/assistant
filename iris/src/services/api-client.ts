...
export class MilesApiClient {
  ...
  async getRoomAvailability(params: {
    ...
    if (error) {
      throw new Error((error as ApiError).error || 'Failed to check availability');
    }
  ...
  async updateBooking(bookingId: string, updates: Partial<BookingInput>): Promise<BookingResponse> {
