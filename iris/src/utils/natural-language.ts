export interface ParsedIntent {
  type: 'greeting' | 'rooms_query' | 'bookings_query' | 'availability_check' | 'booking_create' | 'cancel_all' | 'llm_fallback' | 'unknown';
  entities: {
    roomName?: string;
    time?: string;
    duration?: string;
...
export class NaturalLanguageProcessor {
  ...
  parseIntent(input: string): ParsedIntent {
    ...
    if (this.bookingQueryPatterns.some(pattern => pattern.test(trimmedInput))) {
      return { 
        type: 'bookings_query', 
        entities: {}, 
        confidence: 0.8, 
